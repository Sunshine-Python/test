...
class ATRStrategy(Strategy):
    ...
    def init(self):
        self.atr = self.I(calculate_atr, self.data.High, self.data.Low, self.data.Close, self.period)
    ...
    def next(self):
        ...
        if self.position:
            if self.position_type == 'long':
                if self.enable_stop_loss and current_price <= self.entry_price * (1 - self.stop_loss_pct / 100):
                    ...
                    self.position.close()
                    self.position_type = None
                elif current_price < prev_close - self.multiplier * current_atr:
                    self.position.close()
                    self.position_type = None
        ...
        if not self.position:
            if current_price > prev_close + self.multiplier * current_atr:
                self.buy()
                self.entry_price = current_price
                self.position_type = 'long'
            elif current_price < prev_close - self.multiplier * current_atr and self.enable_shorting:
                self.sell()
                self.entry_price = current_price
                self.position_type = 'short'

def atr_viz(data, period=14, multiplier=2):
    ...
    atr = calculate_atr(data['High'].values, data['Low'].values, data['Close'].values, period)
    data['ATR'] = atr
    data['Upper'] = data['Close'].shift(1) + multiplier * data['ATR']
    data['Lower'] = data['Close'].shift(1) - multiplier * data['ATR']
...
def run_atr_strategy(ticker, start_date, end_date, cash, commission, period, multiplier, stop_loss_pct, take_profit_pct, enable_shorting, enable_stop_loss, enable_take_profit):
    ATRStrategy.period = period
    ATRStrategy.multiplier = multiplier
