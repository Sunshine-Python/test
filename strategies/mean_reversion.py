...
class MeanReversion(Strategy):
    n = 20  # lookback period for calculating mean
    entry_std = 2.0  # number of standard deviations for entry
    exit_std = 0.5  # number of standard deviations for exit
    stop_loss_pct = 2.0
    take_profit_pct = 5.0
...
def mean_reversion_viz(data, n=20, entry_std=2.0, exit_std=0.5):
    ...
    data['Upper_Entry'] = data['SMA'] + entry_std * data['STD']
    data['Lower_Entry'] = data['SMA'] - entry_std * data['STD']
    data['Upper_Exit'] = data['SMA'] + exit_std * data['STD']
    data['Lower_Exit'] = data['SMA'] - exit_std * data['STD']
...
def run_mean_reversion(ticker, start_date, end_date, cash, commission, n, entry_std, exit_std, stop_loss_pct, take_profit_pct, enable_shorting, enable_stop_loss, enable_take_profit):
    MeanReversion.n = n
    MeanReversion.entry_std = entry_std
    MeanReversion.exit_std = exit_std
