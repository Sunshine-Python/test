...
class VWAPStrategy(Strategy):
    ...
    def init(self):
        self.vwap = self.I(lambda: self.calculate_vwap(self.n_periods))
...
def vwap_viz(data, n_periods=20):
    ...
    cumulative_pv = (data['Close'] * data['Volume']).rolling(window=n_periods).sum()
    cumulative_volume = data['Volume'].rolling(window=n_periods).sum()
    ...
    ax.plot(data.index, vwap, label=f'VWAP({n_periods})', color='red')
...
def run_vwap_strategy(ticker, start_date, end_date, cash, commission, n_periods, stop_loss_pct, take_profit_pct, enable_shorting, enable_stop_loss, enable_take_profit):
    VWAPStrategy.n_periods = n_periods
