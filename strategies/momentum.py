...
class MomentumStrategy(Strategy):
    ...
    def init(self):
        ...
        self.roc = self.I(self.calculate_roc, close, self.n_period)
    ...
    def next(self):
        ...
        if not self.position:
            if self.roc[-1] > self.threshold:
                self.buy()
                self.entry_price = self.data.Close[-1]
                self.position_type = 'long'
            elif self.roc[-1] < -self.threshold and self.enable_shorting:
                self.sell()
                self.entry_price = self.data.Close[-1]
                self.position_type = 'short'

def momentum_viz(data, n_period=14, threshold=0):
    ...
    roc = (data['Close'] / data['Close'].shift(n_period) - 1) * 100
    ...
    ax2 = ax1.twinx()
    ax2.plot(data.index[n_period:], roc[n_period:], label=f'ROC({n_period})', color='orange')
...
def run_momentum(ticker, start_date, end_date, cash, commission, n_period, threshold, stop_loss_pct, take_profit_pct, enable_shorting, enable_stop_loss, enable_take_profit):
    MomentumStrategy.n_period = n_period
    MomentumStrategy.threshold = threshold
