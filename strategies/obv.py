...
class OBVStrategy(Strategy):
    ...
    def init(self):
        ...
        self.obv_sma = self.I(lambda: pd.Series(self.obv).rolling(self.n_periods).mean())
...
def obv_viz(data, n_periods=20):
    ...
    obv_sma = pd.Series(obv).rolling(window=n_periods).mean()
    ...
    ax2.plot(data.index, obv_sma, label=f'OBV SMA({n_periods})', color='red')
    ax2.set_xlabel('Time')
    ax2.set_ylabel('OBV')
    ax2.legend()
    ax2.grid(True)
...
def run_obv_strategy(ticker, start_date, end_date, cash, commission, n_periods, stop_loss_pct, take_profit_pct, enable_shorting, enable_stop_loss, enable_take_profit):
    OBVStrategy.n_periods = n_periods
